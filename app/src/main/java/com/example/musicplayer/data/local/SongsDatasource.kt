...
class SongsDatasource @Inject constructor(
    @ApplicationContext val context: Context, coroutineScope: CoroutineScope
) {

    private val audioColumns = listOf(
        Media.TITLE, Media._ID, Media.ALBUM, Media.ARTIST, Media.DURATION
    )
    ...
    private fun queryMediaStore(): Cursor? {
        ...
        return context.contentResolver.query(
            Media.EXTERNAL_CONTENT_URI, audioColumns.toTypedArray(), filterMusics, null, null
        )
    }
    ...
    private fun fetchAllSongs(cursor: Cursor): List<Song> {
        ...
        while (cursor.moveToNext()) with(cursor) {
            ...
            songsList.add(Song(id, title, album, artist, duration, contentUri))
        }
        return songsList
    }
